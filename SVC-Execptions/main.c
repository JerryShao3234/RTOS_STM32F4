...
__asm void SVC_Handler (void) {
	...
	TST LR, #4
	ITE EQ
	MRSEQ R0, MSP //go to main stack pointer
	MRSNE R0, PSP  // go to processor stack pointer
...
void SVC_Handler_C(unsigned int * svc_args) {
	...
	switch(svc_number) {
		case 0: 
			svc_args[0] = svc_args[0] + svc_args[1]; //R0 and R1 are function parameters based on ARM architecture
			break;
		case 1: 
